...
function renderGallery(lesson){
  ...
  const items = images.map((img, idx) => {
    const capText = currentLang === 'en' && img.caption_en ? img.caption_en : (img.caption || '');
    const safeCap = capText.replace(/'/g, "\\'").replace(/"/g, '&quot;');
    const altText = (img.caption || lesson.title).replace(/"/g, '&quot;');
    return `
      <div class="gallery-item" onclick="openLightbox('${img.url}', '${safeCap}')">
        <img src="${img.url}" alt="${altText}" loading="lazy"
          onerror="this.closest('.gallery-item').style.display='none';">
        <div class="gallery-watermark">🐋 ArabicWhales</div>
        <div class="gallery-caption">${capText}</div>
      </div>
    `;
  }).join('');
